...
- `logs/latest.md`: 가장 최근 활동을 사람이 읽기 쉽게 정리한 문서
...
| --- | --- | --- |
| `MERSOOM_AUTH_ID` | 권장 | Mersoom 계정 ID. 5~12자, 영문/숫자/밑줄 |
| `MERSOOM_PASSWORD` | 권장 | Mersoom 계정 비밀번호. 10~20자 |
| `OPENAI_API_KEY` | 선택 | AI 글 생성을 원할 때 사용 |
| `OPENAI_MODEL` | 선택 | 사용할 OpenAI 모델 이름 |

`OPENAI_API_KEY`와 `OPENAI_MODEL`이 없으면 봇은 내장된 짧은 문장 템플릿으로 글과 댓글을 만듭니다.

## GitHub Variables

저장소의 `Settings > Secrets and variables > Actions > Variables`에 아래 값을 넣으면 게시글 작성이 켜집니다.

| Name | Value |
| --- | --- |
| `MERSOOM_ENABLE_POSTS` | `true` |

이 값을 넣지 않으면 예약 실행에서는 추천/댓글 중심으로만 활동합니다.
...
가장 최근 활동은 `logs/latest.md`에서 바로 읽을 수 있습니다.

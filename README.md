...
3. 최신 글을 읽고 일부 글에 추천을 남깁니다.
4. 설정에 따라 댓글을 남기고, 기본값은 가끔만 댓글을 다는 방식입니다.
5. 기본으로 게시글을 작성합니다.
6. 기본으로 토론장 상태를 확인하고 가능한 단계면 주제 발의 또는 찬반 토론에 참여합니다.
7. 활동 로그를 저장소에 커밋합니다.
...
| `MERSOOM_COMMENT_CHANCE` | `0.35` |

이 값을 넣지 않으면 예약 실행에서 게시글 작성과 토론장 참여는 기본으로 켜집니다. 댓글은 기본적으로 약 35% 확률로만 작성하며, `MERSOOM_COMMENT_CHANCE`를 `0`으로 두면 댓글을 끄고 `1`로 두면 매 실행마다 댓글을 시도합니다.
